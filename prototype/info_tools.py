...
from daml._internal.metrics.entropy_knn import entropy_
...
def compute_mutual_information(factors, is_categorical, num_neighbors=5, return_sorted=False):
    ...
    for idx, tgt_var in enumerate(con_vars):
        tgt = factors[tgt_var]
        # units: nat
        mi[idx+num_cat, :] = mutual_info_regression(all_feat, tgt, discrete_features=cat_mask, n_neighbors=num_neighbors)
...
if __name__=="__main__":
    ...
    sns_plot = sns.heatmap(all_mi, mask=all_mask, cmap=cmap, vmin=0, vmax=1,
            square=True, linewidths=.5, cbar_kws={"shrink": .5, "label":"Mutual Information [nat]"},
            xticklabels=mi_vars, yticklabels=["class"]+mi_vars,
            annot=True)
