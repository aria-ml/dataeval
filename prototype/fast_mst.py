# Adapted from fast_hdbscan and multi_mst python modules
# Original Authors: fast_hdbscan: Leland McInnes <https://github.com/TutteInstitute/fast_hdbscan>
#                   multi_mst: Jelmer Bot <https://github.com/vda-lab/multi_mst>
...
@numba.njit()
def _ds_rank_create(n_elements):
    return RankDisjointSet(np.arange(n_elements, dtype=np.uint32), np.zeros(n_elements, dtype=np.uint16))

@numba.njit()
def _ds_find(disjoint_set, x):
...
def _ds_union_by_rank(disjoint_set, point, nbr):
    y = _ds_find(disjoint_set, point)
    x = _ds_find(disjoint_set, nbr)
...
def _init_tree(n_neighbors, n_distance):
    ...
    disjoint_set = _ds_rank_create(n_neighbors.size)
    ...
    for i in range(cluster_points.size):
        cluster_points[i] = _ds_find(disjoint_set, i)
...
def _cluster_edges(tracker, last_idx, cluster_distances):
    ...
    edge_points = []
    for id in range(cluster_ids.size):
        cluster_points = np.nonzero(tracker==cluster_ids[id])[0]
...
def minimum_spanning_tree(data, neighbors, distances):
    ...
    for i in range(1,k_neighbors.shape[0]):
        tree, int_tree, tree_disjoint_set, merge_tracker[i] = _update_tree_by_distance(tree, int_tree, tree_disjoint_set, k_neighbors[i], k_distances[i])
        last_idx = i
    ...
    if cluster_ids.size > 1:
        ...
        additional_neighbors, additional_distances = _calculate_cluster_neighbors(data, edge_points, merge_tracker[last_idx])

        # Update clusters
        last_idx += 1
        tree, int_tree, tree_disjoint_set, merge_tracker[last_idx] = _update_tree_by_distance(tree, int_tree, tree_disjoint_set, additional_neighbors, additional_distances)
