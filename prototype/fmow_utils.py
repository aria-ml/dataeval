...
def extrinsic_factors_fmow(df):
    ...
    vars = ["utm", "country_code", "cloud_cover","target_azimuth_dbl","sun_azimuth_dbl","sun_elevation_dbl","off_nadir_angle_dbl"]
    for v in vars:
        ext[v] = df[v].to_numpy()
    ...
    assert(all([k in is_categorical.keys() for k in ext.keys()]))
