...
@pytest.mark.skip()
@pytest.mark.parametrize("split_on", groupnames)
@pytest.mark.parametrize("test_frac", [None, 0.75])
...
@pytest.mark.parametrize("split_on", groupnames)
@pytest.mark.parametrize("test_frac", [None, 0.6])
...
class TestDatasetSplitter:
    def test_invalid_args(self, labels_with_metadata, nfolds, test_frac, val_frac, split_on, stratify):
        ...
        if val_frac and (nfolds not in [None, 1]):
            ...
            with pytest.raises(UnboundLocalError):
                split_dataset(labels, 1, test_frac, 0.125, split_on, None, stratify)  # type: ignore
        else:
