...
def test_aegmm(aegmm_params):
    ...
    # init OutlierAEGMM
    aegmm = AEGMMOutlier(AEGMM(encoder_net, decoder_net, gmm_density_net, n_gmm))
    ...
    assert od_preds["is_outlier"].sum() == (od_preds["instance_score"] > aegmm._threshold_score()).astype(int).sum()
