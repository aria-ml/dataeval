...
from dataeval.utils.tensorflow.internal.autoencoder import (
...
def test_relative_euclidean_distance():
    ...
    assert (
        relative_euclidean_distance(x, y).numpy()
        == relative_euclidean_distance(y, x).numpy()
    ).all()  # type: ignore
    assert (
        relative_euclidean_distance(x, x).numpy()
        == relative_euclidean_distance(y, y).numpy()
    ).all()  # type: ignore
