...
def test_gmm_params_energy(module, tz, tg):
    params = module.gmm_params(z, gamma)
    ...
    sample_energy, cov_diag = module.gmm_energy(z, params, return_mean=True)
    assert sample_energy.numpy().shape == cov_diag.numpy().shape == ()  # type: ignore

    sample_energy, _ = module.gmm_energy(z, params, return_mean=False)
