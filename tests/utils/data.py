...
class DataEvalWithMetadataDataset(Dataset):
    """Holds the arrays of images, labels, bounding boxes, and metadata"""
    ...
    def __getitem__(self, index) -> Any | tuple:
        ...
        # Return image and label for image classification
        if self._boxes is None:
            output: Any = [image, labels]
        # Return images, labels, boxes for object detection
        output: Any = [image, labels, self._boxes[index]]
        if self._metadata is not None:
            output.append(self._metadata[index])
        else:
            output.append({})
        return tuple(output)
    ...
    def _validate(self):
        if self._labels is not None:
            if self._boxes is not None:
                ...
                assert len(self._images) == len(self._labels)
            if self._metadata is not None:
                assert len(self._metadata) == len(self._labels)
        else:
            if self._metadata is not None:
                assert len(self._metadata) == len(self._images)
            assert self._boxes is None
