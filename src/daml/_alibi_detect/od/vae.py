...
from daml._alibi_detect.models.tensorflow.losses import elbo
...
class OutlierVAE(BaseDetector, FitMixin, ThresholdMixin):
    def __init__(
        ...
        if isinstance(vae, tf.keras.Model):
            ...
            self.vae = VAE(
                encoder_net, decoder_net, latent_dim, beta=beta
            )  # define VAE model
        else:
            raise TypeError(
    ...
    def instance_score(
        self, fscore: np.ndarray, outlier_perc: float = 100.0
    ) -> np.ndarray:
        """
