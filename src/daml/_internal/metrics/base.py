...
class OutlierDetector(Metric, ABC):
    ...
    def check_dataset_shape(self, dataset, reference_input_shape):
        ...
        ------
        TypeError
            If the dims of the last 3 channels of dataset
            are not the same as the reference_input_shape
        """
        if dataset.shape[-3:] != reference_input_shape:
            raise TypeError(
                f"Model was initialzied on dataset shape \
                (H, W, C)={reference_input_shape}, \
                but provided dataset has shape (H, W, C)={dataset.shape[-3:]}"
            )
