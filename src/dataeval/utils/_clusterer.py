...
from dataeval.utils._fast_mst import ds_union_by_rank


@numba.njit(parallel=True, locals={"i": numba.types.int32})
def group_mst_by_clusters(mst, clusters):
...
@numba.njit()
def cluster_variance(clusters, mst_clusters, mst):
...
@numba.njit(parallel=True, locals={"i": numba.types.int32})
def compare_links_to_std(cluster_std, mst_clusters, mst):
