...
from dataeval.utils.tensorflow.internal.gmm import gmm_energy, gmm_params
...
class Elbo:
    ...
    def __init__(
        ...
        if isinstance(cov_type, float):
            ...
            x_np: NDArray[np.float32] = (
                x.numpy().astype(np.float32) if tf.is_tensor(x) else x
            )  # type: ignore
            cov = covariance(x_np.reshape(x_np.shape[0], -1))  # type: ignore py38
            if (
                cov_type == "cov_diag"
            ):  # infer standard deviation from covariance matrix
                cov = tf.math.sqrt(tf.linalg.diag_part(cov))
            self._cov = (cov_type, cov)
        else:
            raise ValueError(
                "Only cov_full, cov_diag or sim value should be specified."
            )
    ...
    def __call__(self, y_true: tf.Tensor, y_pred: tf.Tensor) -> tf.Tensor:
        ...
        if self._cov[0] == "cov_full":
            y_mn = MultivariateNormalTriL(
                y_pred_flat, scale_tril=tf.linalg.cholesky(self._cov[1])
            )
        else:  # cov_diag and sim
            cov_diag = (
                self._cov[1]
                if self._cov[0] == "cov_diag"
                else self._cov[1] * tf.ones(y_pred_flat.shape[-1])
            )
