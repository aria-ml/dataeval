# ruff: noqa
"""Hierarchical clustering tree operations with Numba JIT compilation.

This module is adapted from fast_hdbscan.cluster_trees v0.2.2:
...
Modifications:
    ...
    - Updated disjoint set imports to use local cached implementations
"""
...
from dataeval.core._fast_hdbscan._disjoint_set import ds_rank_create, ds_find, ds_union_by_rank

from numba.typed import Dict, List
from numba.types import int64, ListType  # type: ignore
...
def eliminate_branch(
    branch_node, parent_node, lambda_value, parents, children, lambdas, sizes, idx, ignore, hierarchy, num_points
):
    ...
    for node in node_list:
        ...
        # Next most common is a small left cluster and a large right cluster: relabel the right node; eliminate the left branch
            ...
            idx = eliminate_branch(
                left, parent_node, lambda_value, parents, children, lambdas, sizes, idx, ignore, hierarchy, num_points
            )
        # Then we have a large left cluster and a small right cluster: relabel the left node; eliminate the right branch
            ...
            idx = eliminate_branch(
                right, parent_node, lambda_value, parents, children, lambdas, sizes, idx, ignore, hierarchy, num_points
            )
...
def get_condensed_tree_clusters_bcubed(
    condensed_tree, label_indices, cluster_tree=None, cluster_tree_bcubed=None, allow_virtual_nodes=False
):
    ...
    for i in range(len(condensed_tree.child) - 1, -1, -1):  # Traverse tree bottom up
        ...
        if child in parents_set:
            if parent in cluster_elements:
                ...
                cluster_elements[parent] = List(cluster_labeled_points)  # type: ignore
...
def eom_recursion_bcubed(
    node, cluster_tree, stability_node_scores, bcubed_node_scores, selected_clusters, unselected_nodes
):
    ...
    for child_node in children:
        child_score_total_stability_bcubed += eom_recursion_bcubed(
            child_node, cluster_tree, stability_node_scores, bcubed_node_scores, selected_clusters, unselected_nodes
        )  # type: ignore
...
def extract_clusters_bcubed(
    condensed_tree, cluster_tree, data_labels, allow_virtual_nodes=False, allow_single_cluster=False
):
    label_indices = Dict()  # type: ignore
    ...
    if allow_virtual_nodes:
        ...
        cluster_elements, virtual_nodes = get_condensed_tree_clusters_bcubed(
            condensed_tree, label_indices, cluster_tree, cluster_tree_bcubed, allow_virtual_nodes
        )
...
def eom_recursion(node, cluster_tree, node_scores, node_sizes, selected_clusters, max_cluster_size):
    ...
    for child_node in children:
        child_score_total += eom_recursion(
            child_node, cluster_tree, node_scores, node_sizes, selected_clusters, max_cluster_size
        )
...
def remap_cluster_ids(condensed_tree, n_points):
    ...
    id_map[remaining_parents - n_points] = np.arange(n_points, n_points + remaining_parents.shape[0])
...
def get_cluster_labelling_at_cut(linkage_tree, cut, min_cluster_size):
    ...
    for n in range(num_points):
        cluster = ds_find(disjoint_set, np.intp(n))
...
def min_lambdas(cluster_tree, clusters):
    return {c: cluster_tree.lambda_val[np.searchsorted(cluster_tree.child, c)] for c in clusters}
