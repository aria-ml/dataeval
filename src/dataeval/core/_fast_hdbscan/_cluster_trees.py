...
This module is adapted from fast_hdbscan.cluster_trees v0.2.0:
...
def get_condensed_tree_clusters_bcubed(
    condensed_tree, cluster_tree=None, cluster_tree_bcubed=None, allow_virtual_nodes=False
):
    ...
    for i in range(len(condensed_tree.child) - 1, -1, -1):  # Traverse tree bottom up
        ...
        if child in parents_set:
            if parent in cluster_elements:
                ...
                cluster_elements[parent] = List(cluster_elements[child])  # type: ignore
...
def eom_recursion_bcubed(node, cluster_tree, stability_node_scores, bcubed_node_scores, selected_clusters):
    ...
    for child_node in children:
        child_score_total_stability_bcubed += eom_recursion_bcubed(
            child_node, cluster_tree, stability_node_scores, bcubed_node_scores, selected_clusters
        )  # type: ignore
...
def extract_clusters_bcubed(
    condensed_tree, cluster_tree, label_indices, allow_virtual_nodes=False, allow_single_cluster=False
):
    if allow_virtual_nodes:
        ...
        cluster_elements, virtual_nodes = get_condensed_tree_clusters_bcubed(
            condensed_tree, cluster_tree, cluster_tree_bcubed, allow_virtual_nodes
        )
        stability_node_scores = score_condensed_tree_nodes(condensed_tree)
        for node in virtual_nodes:
            stability_node_scores[node] = 0.0  # type: ignore
    ...
    if allow_single_cluster:
        eom_recursion_bcubed(
            cluster_tree_root, cluster_tree_bcubed, stability_node_scores, bcubed_node_scores, selected_clusters
        )
        ...
        for child_node in root_children:
            eom_recursion_bcubed(
                child_node, cluster_tree_bcubed, stability_node_scores, bcubed_node_scores, selected_clusters
            )
...
def segments_in_branch(cluster_tree, segment):
    ...
    while len(to_process) > 0:
        result |= to_process
        to_process = set(cluster_tree.child[in_set_parallel(cluster_tree.parent, to_process)])
...
def in_set_parallel(values, targets):
    mask = np.empty(values.shape[0], dtype=numba.boolean)  # type: ignore
